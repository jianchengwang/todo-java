package opencv.imageSimilarity.ssim;
...
import opencv.imageSimilarity.Utils;

/**
 * ssim 基于图像结构的图像相似度算法
 * git地址: https://github.com/rhys-e/structural-similarity
 * @author xiezhengbiao
 *
 */
public class SsimCalculator {
    ...
    public SsimCalculator(File referenceFile) throws Exception {
        this.refImage = getBufferedImageForBytes(loadFile(referenceFile));
    }

    public SsimCalculator(byte[] data) throws Exception {
        this.refImage = getBufferedImageForBytes(data);
    }
    ...
    public double compareTo(File comp) throws Exception {
    ...
    private byte[] loadFile(File fileToLoad) throws Exception {
        return Utils.readStream(fileToLoad.getAbsolutePath());
    }
}
